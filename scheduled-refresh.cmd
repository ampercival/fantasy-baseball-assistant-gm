@echo off
REM Unattended full refresh (all sources + leagues) -> Supabase.
...
echo ============================================================>> "logs\scheduled-refresh.log"
echo [%DATE% %TIME%] Starting scheduled refresh (all sources + leagues)>> "logs\scheduled-refresh.log"
".venv\Scripts\python.exe" backend\refresh_supabase.py >> "logs\scheduled-refresh.log" 2>&1
echo [%DATE% %TIME%] Exit code %ERRORLEVEL%>> "logs\scheduled-refresh.log"
exit /b %ERRORLEVEL%
