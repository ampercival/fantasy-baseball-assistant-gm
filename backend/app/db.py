...
def get_metadata(conn: Connection, key: str) -> str | None:
    row = conn.execute("SELECT value FROM app_metadata WHERE key = ?", (key,)).fetchone()
    return row["value"] if row else None


def set_metadata(conn: Connection, key: str, value: str) -> None:
    conn.execute(
        """
...
def maybe_normalize_player_keys(conn: Connection) -> None:
