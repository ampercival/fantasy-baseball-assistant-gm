"""Operator refresh: scrape ranking sources, connected leagues, and the Ottoneu platform curve.
...
Usage (from the project root, with the virtualenv + .env DATABASE_URL in place):
    python backend/refresh_supabase.py                # sources + leagues + platform curve
    python backend/refresh_supabase.py --continuous   # Continuous sources + leagues + platform curve
